    ...
    return (
        <div className={styles.container}>
            ...
            <ul className={styles.instructionsList}>
                <li className={styles.instructionsItem}><BsCheck2Circle
                    className={styles.icon}/>{isEnglish ? 'Read the text once (without marking).' : 'Διαβάστε μία φορά το κείμενο (χωρίς επισημείωση).'}
                </li>
                <li className={styles.instructionsItem}><BsCheck2Circle
                    className={styles.icon}/>{isEnglish ? 'Read the text a second time looking for words/phrases that clearly speak of a value.' : 'Διαβάστε δεύτερη φορά το κείμενο εντοπίζοντας λέξεις/φράσεις που ξεκάθαρα μιλούν για μια αξία.'}
                </li>
                <li className={styles.instructionsItem}><BsCheck2Circle
                    className={styles.icon}/>{isEnglish ? 'Read the text a third time finding words/phrases that indirectly talk about a value. For example the phrase: "... is always fully informed about current developments" is associated with the value "knowledge".' : 'Διαβάστε τρίτη φορά το κείμενο εντοπίζοντας λέξεις/φράσεις που έμμεσα μιλούν για μια αξία. Για παράδειγμα  η φράση:  “... είναι πάντα πλήρως ενημερωμένη για τις τρέχουσες εξελίξεις ” συνδέεται με την αξία “γνώση”.'}
                </li>
                <li className={styles.instructionsItem}><BsCheck2Circle
                    className={styles.icon}/>{isEnglish ? 'Capture your first impression - there is no right or wrong interpretation' : 'Αποτυπώστε την πρώτη σας εντύπωση - δεν υπάρχει σωστή ή λάθος ερμηνεία.'}
                </li>
            </ul>
